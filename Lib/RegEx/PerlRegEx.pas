...
begin
  if FRegEx = '' then
    raise Exception.Create(e_Custom, 'TPerlRegEx.Compile() - Please specify a regular expression in RegEx first');
  CleanUp;
  Pattern := pcre_compile(PAnsiChar(FRegEx), pcreOptions, @Error, @ErrorOffset, chartable);
  if Pattern = nil then
    raise Exception.Create(e_Custom, Format('TPerlRegEx.Compile() - Error in regex at offset %d: %s', [ErrorOffset, AnsiString(Error)]));
  FCompiled := True
end;
...
begin
  ...
  if Error <> nil then
    raise Exception.Create(e_Custom, 'TPerlRegEx.Study() - Error studying the regex: ' + AnsiString(Error));
  FStudied := True
end;
...
begin
  Result := TPerlRegEx(Pointer(FList.Items[Index]));
end;
