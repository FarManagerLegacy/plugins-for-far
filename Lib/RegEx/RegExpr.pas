...
{
    Patched by Andrew V. Andreenkov for compatible with
...
const
  ...
  RegExprModifierU : boolean = False;    // default value for ModifierU
...
{$ENDIF}
 ...
 TRegExpr = class
   ...
   public
    ...
    property ModifierU : boolean index 4 read GetModifier write SetModifier;
    // Switching on modifier /u switchs all operators in
    // non-greedy style, so if ModifierU = True, then
    // all '*' works as '*?', all '+' as '+?' and so on.
    // , initialized from RegExprModifierU
...
function RegExprSubExpressions (const ARegExpr : string;
 ASubExprs : PStrListEx; AExtendedSyntax : boolean{$IFDEF DefParam}= False{$ENDIF}) : integer;
...
//  SetLength (Stack, StackSz); //###0.945
         ...
         then Result := i // unbalanced ')'
           ...
           with Stack [StackIdx] do begin
             ...
             ASubExprs.Objects [SubExprIdx] :=
              Cardinal (StartPos or (SubExprLen ShL 16));
  ...
  if (ASubExprs.Count = 0)
   ...
   or (((integer (ASubExprs.Objects [0]) ShR 16) and $FFFF) <> Len)
    ...
    // well, we add it now:
    then ASubExprs.InsertObject (0, ARegExpr, Cardinal ((Len ShL 16) or 1));
 ...
 begin
  ...
  case op of
    '*': begin
      ...
      NonGreedyOp := NonGreedyCh or ((fCompModifiers and MaskModU) <> 0); //###0.940
 ...
 begin
     ...
     if op = ANYOFFULLSET then begin
       for Ch := #0 to #255 do
        if Ch in PSetOfREChar (s)^ then
         if Ch < ' '
          then Result := Result + '#' + Int2Str (Ord (Ch)) //###0.936
  ...
  for Ch := #0 to #255 do
   if Ch in FirstCharSet
    then begin
      if Ch < ' '
       then Result := Result + '#' + Int2Str(Ord(Ch)) //###0.948
       else Result := Result + Ch;
    end;
  {$ENDIF}
  Result := Result + #$d#$a;
...
{$ENDIF}
 ...
 begin
  ...
  if AErrorID < 1000 // compilation error ?
   then e := ERegExpr.Create (err.e_Abort, ErrorMsg (AErrorID) // yes - show error pos
             + ' (pos ' + Int2Str (CompilerErrorPos) + ')')
   else e := ERegExpr.Create (err.e_Abort, ErrorMsg (AErrorID));
